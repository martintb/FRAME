    ...
    def sample(self, num_samples: int, device: torch.device, latent_size: Optional[int] = None) -> torch.Tensor:
        ...
        if latent_size is None:
            if self.latent_spatial_size is not None:
                ...
                print(f"UNetVAE.sample: Using cached latent_size={latent_size}³ from forward pass")
            else:
                # Fallback to computation (assumes 128^3 input)
                latent_size = 128 // (2 ** self.levels)
                print(f"UNetVAE.sample: WARNING - Using fallback latent_size={latent_size}³ (assumes 128³ input)")
            ...
            print(f"UNetVAE.sample: Using explicit latent_size={latent_size}³")
        ...
        print(f"UNetVAE.sample: Sampling from N(0,1) with shape {z.shape}")
